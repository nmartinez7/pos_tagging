...
def train(data_path):
    ...
    for sample in read_jsonl(data_path):
        ...
        for tag, tok in zip(tags,tokens):
            if tag not in collec[tok]:
                collec[tok][tag] = 1
            else:
                collec[tok][tag] = collec[tok][tag] + 1
    ...
    most_common_tag = max(dict_tags, key=dict_tags.get)

    for tok in collec.keys():
        most_common = Counter(collec[tok]).most_common(1)[0]
        final[tok] = most_common
    ...
    '''
    model_params = tuple((final, most_common_tag))
...
def predict_tag(token, params):
    ...
    if token not in params[0]:
        tag_pred = params[1]
        tag_count = 0

    else:
        tag_pred = params[0][token][0]
        tag_count = params[0][token][1]

    return tuple((tag_pred, tag_count))
